...
if (!is_null($con)){
    ...
    if (isset($obj->id_dojo)) {
        $id_dojo = $obj->id_dojo;
        $query_mentor = sprintf("select log_mentor.date, dojo.id as id_dojo, dojo.name as dojo_name, mentor.id as id_mentor, contact.name as mentor_name, contact.lastname as mentor_lastname from log_mentor inner join dojo on dojo.id = log_mentor.id_dojo inner join mentor on mentor.id = log_mentor.id_mentor inner join contact on contact.id = mentor.id_contact where log_mentor.id_dojo = '%s'",$con->real_escape_string($id_dojo));
        $query_padawan = sprintf("select log_padawan.date, dojo.id as id_dojo, dojo.name as dojo_name, padawan.id as id_padawan, contact.name as padawan_name, contact.lastname as padawan_lastname from log_padawan inner join dojo on dojo.id = log_padawan.id_dojo inner join padawan on padawan.id = log_padawan.id_padawan inner join contact on contact.id = padawan.id_contact where log_padawan.id_dojo = '%s'",$con->real_escape_string($id_dojo));
        ...
        if (($result_mentor)&&($result_padawan)) {
                ...
                while ($rowMentors = $result_mentor->fetch_array(MYSQLI_ASSOC)) {
                    $mentors [] = array('date' => $rowMentors['date'],
                ...
                while ($rowPadawans = $result_padawan->fetch_array(MYSQLI_ASSOC)) {
                    $padawans [] = array('date' => $rowPadawans['date'],
        ...
        if (isset($obj->id_mentor)) {
            $query = sprintf("select log_mentor.date, dojo.id as id_dojo, dojo.name as dojo_name, mentor.id as id_mentor, contact.name as mentor_name, contact.lastname as mentor_lastname from log_mentor inner join dojo on dojo.id = log_mentor.id_dojo inner join mentor on mentor.id = log_mentor.id_mentor inner join contact on contact.id = mentor.id_contact where log_mentor.id_mentor = '%s'",$con->real_escape_string($obj->id_mentor));
            $result = $con->query($query);
            if ($result) {
                $json = array();
                while ($row = $result->fetch_array(MYSQLI_ASSOC)) {
                    $json [] = array(   'date' => $row['date'],
                                        'id_dojo' => $row['id_dojo'],
