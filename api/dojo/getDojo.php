...
if (!is_null($con)){
    ...
    if (isset($obj->id_dojo))
        $query = sprintf("select dojo.id, dojo.name, dojo.address, dojo.city, dojo.description, dojo.phone, dojo.email, dojo.facebook, dojo.twitter, status.name as status, dojo.id_status from dojo left join status on status.id = dojo.id_status WHERE dojo.id = '%s'",$con->real_escape_string($obj->id_dojo));
    else
