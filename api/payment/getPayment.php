...
if (!is_null($con)){
    ...
    if (isset($obj->id_payment))
        $query = sprintf("select payment.id, payment.date, payment.month, payment.year, payment.amount, payment.observation, payment.id_dojo, dojo.name as name_dojo, payment.id_padawan, contact.name as name_padwan, contact.lastname as lastname_padawan from payment inner join dojo on dojo.id = payment.id_dojo inner join padawan on padawan.id = payment.id_padawan inner join contact on contact.id = padawan.id_contact where payment.id = '%s'",$con->real_escape_string($obj->id_payment));
        ...
        $query = "select payment.id, payment.date, payment.month, payment.year, payment.amount, payment.observation, payment.id_dojo, dojo.name as name_dojo, payment.id_padawan, contact.name as name_padwan, contact.lastname as lastname_padawan from payment inner join dojo on dojo.id = payment.id_dojo inner join padawan on padawan.id = payment.id_padawan inner join contact on contact.id = padawan.id_contact order by payment.date desc";
